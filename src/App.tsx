...
const App = () => {
  ...
  const availableTimeSlots = useMemo(() => {
    const slots = [];
    const now = new Date();
    const isToday = new Date(selectedDate + 'T00:00:00').getTime() === today.getTime();

    const isSlotBooked = (slot: string) => bookings.some(booking => slot >= booking.startTime && slot < booking.endTime);
    ...
    return (
      <div className={`${sizeClasses[size]} relative flex-shrink-0`}>
        <svg viewBox="0 0 100 100" className="w-full h-full">
          <defs><linearGradient id="azrachit-gradient" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#7DD3FC" /><stop offset="100%" stopColor="#0EA5E9" /></linearGradient></defs>
  ...
  return (
    <div className="min-h-screen bg-slate-100 font-sans">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row justify-between items-center py-4 gap-4 sm:gap-0">
            ...
            <div className="flex items-center space-x-2 sm:space-x-4 w-full sm:w-auto">
              ...
              <button
                onClick={() => { setShowBookingForm(true); setBookingForm({ title: '', startTime: '', endTime: '', attendees: '', notes: '' }); }}
                disabled={loading}
              ...
              >
                <Plus className="w-5 h-5" /><span className="hidden sm:inline">Book Room</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {notification && (
        <div className={`fixed top-24 sm:top-20 right-4 z-[60] flex items-center space-x-3 px-4 py-3 rounded-lg shadow-lg ${notification.type === 'success' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}>
          {notification.type === 'success' ? <Check className="w-5 h-5" /> : <AlertCircle className="w-5 h-5" />}<span>{notification.message}</span>
        </div>
      )}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white/60 backdrop-blur-xl rounded-2xl shadow-lg p-6 mb-8 border border-slate-200/80">
          <div className="flex flex-col md:flex-row justify-between items-start gap-6">
            <div className="w-full md:w-auto"><div className="flex items-center space-x-3 mb-3"><Building2 className="w-7 h-7 text-blue-600 flex-shrink-0" /><h2 className="text-xl sm:text-2xl font-bold text-slate-800">{meetingRoom.name}</h2></div><div className="pl-10 space-y-2 text-slate-600"><div className="flex items-center space-x-2 text-sm"><Users className="w-4 h-4 text-slate-500" /><span>Capacity: {meetingRoom.capacity} people</span></div><div className="flex items-center space-x-2 text-sm"><MapPin className="w-4 h-4 text-slate-500" /><span>{meetingRoom.location}</span></div></div><div className="pl-10 mt-4 flex flex-wrap gap-2">{meetingRoom.amenities.map((amenity, index) => (<div key={amenity} className="flex items-center space-x-2 px-3 py-1 bg-blue-50 text-blue-700 text-xs font-semibold rounded-full border border-blue-200">{index === 0 ? <Snowflake className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}<span>{amenity}</span></div>))}</div></div>
            <div className="text-center bg-slate-50/80 p-4 rounded-lg border border-slate-200 w-full md:w-auto flex-shrink-0"><div className="text-4xl font-bold text-blue-600">{getDateBookings().length}</div><div className="text-xs text-slate-500 font-medium uppercase tracking-wider">Bookings</div></div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6 border border-slate-200/80">
          ...
          <div className="space-y-4">
            ...
            : getDateBookings().length === 0 ? (
              <div className="text-center py-12 text-slate-400"><Calendar className="w-12 h-12 mx-auto text-slate-300 mb-2" /><p className="font-medium">No bookings for this date.</p><p className="text-sm">The room is available all day.</p></div>
            ) : (
              getDateBookings().map((booking) => (
                <div key={(booking as any).id} className="bg-slate-50 rounded-lg p-4 border border-slate-200 hover:border-blue-400 hover:shadow-sm transition-all duration-300">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1"><h3 className="font-bold text-slate-800">{(booking as any).title}</h3><div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 text-sm text-slate-600 mt-1"><div className="flex items-center space-x-1.5"><Clock className="w-4 h-4 text-blue-500" /><span>{formatTime((booking as any).startTime)} - {formatTime((booking as any).endTime)}</span></div>{(booking as any).attendees && <div className="flex items-center space-x-1.5 mt-1 sm:mt-0"><Users className="w-4 h-4 text-blue-500" /><span>{(booking as any).attendees} attendees</span></div>}</div></div>
      ...
      {showBookingForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl max-w-md w-full animate-in fade-in-0 zoom-in-95">
            <div className="p-6 border-b border-slate-200 flex justify-between items-center"><h2 className="text-lg font-bold text-slate-800">Book Meeting Room</h2><button onClick={() => setShowBookingForm(false)} className="text-slate-400 hover:text-slate-600 p-1 rounded-full hover:bg-slate-100 transition-colors"><X className="w-5 h-5" /></button></div>
            <div className="p-6 space-y-5">
              <div><label className="block text-sm font-medium text-slate-700 mb-1">Meeting Title *</label><div className="relative"><Type className="w-5 h-5 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" /><input type="text" value={bookingForm.title} onChange={(e) => setBookingForm({ ...bookingForm, title: e.target.value })} className="w-full bg-slate-50 border-slate-300 rounded-md pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., Weekly Sync" /></div></div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={bookingForm.endTime} onChange={(e) => setBookingForm({ ...bookingForm, endTime: e.target.value })} className="w-full bg-slate-50 border-slate-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" disabled={!bookingForm.startTime}>
                    <option value="">Select</option>
                    {(() => {
                      const startIndex = availableTimeSlots.findIndex(t => t === bookingForm.startTime);
                      if (startIndex === -1) return null;
                      
                      let endIndex = availableTimeSlots.length;
                      const nextBooking = bookings.find(b => b.startTime > bookingForm.startTime);
                      if (nextBooking) {
                        const nextBookingIndex = availableTimeSlots.findIndex(t => t >= nextBooking.startTime);
                        if (nextBookingIndex !== -1) {
                          endIndex = nextBookingIndex + 1;
                        }
                      }
                      
                      return availableTimeSlots.slice(startIndex + 1, endIndex).map(time => <option key={time} value={time}>{formatTime(time)}</option>);
                    })()}
                  </select>
                </div>
              </div>
              <div><label className="block text-sm font-medium text-slate-700 mb-1">Attendees</label><div className="relative"><Users className="w-5 h-5 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" /><input type="number" value={bookingForm.attendees} onChange={(e) => setBookingForm({ ...bookingForm, attendees: e.target.value })} className="w-full bg-slate-50 border-slate-300 rounded-md pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder={`Max: ${meetingRoom.capacity}`} min="1" max={meetingRoom.capacity} /></div></div>
              <div><label className="block text-sm font-medium text-slate-700 mb-1">Notes</label><div className="relative"><FileText className="w-5 h-5 text-slate-400 absolute left-3 top-3" /><textarea value={bookingForm.notes} onChange={(e) => setBookingForm({ ...bookingForm, notes: e.target.value })} className="w-full bg-slate-50 border-slate-300 rounded-md pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" rows={3} placeholder="e.g., Agenda items..." /></div></div>
            </div>
            <div className="p-4 bg-slate-50 border-t border-slate-200 flex justify-end space-x-3"><button onClick={() => setShowBookingForm(false)} className="bg-white text-slate-700 py-2 px-4 rounded-lg hover:bg-slate-100 border border-slate-300 transition-all font-semibold active:scale-95">Cancel</button><button onClick={handleBookRoom} disabled={loading} className="bg-gradient-to-br from-blue-500 to-blue-600 text-white py-2 px-4 rounded-lg hover:from-blue-600 hover:to-blue-700 disabled:opacity-50 transition-all font-semibold shadow-md hover:shadow-lg active:scale-95">{loading ? 'Booking...' : 'Confirm Booking'}</button></div>
          </div>
        </div>
      )}
    </div>
